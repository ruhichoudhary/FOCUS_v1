...
def write_sdf(m):
    ...
    w = Chem.SDWriter(DATA + '/ligandpep.sdf')
    w.write(m)
...
def smiles_to_pdbqt_v2(smiles, pdbqt_path):
    ...
    os.system('obabel '+DATA+'/ligandpep.sdf -opdb -h -m')
    os.system('obabel ligandpep.pdb -opdbqt -xh -m')

def getpepdata(config, smiles):
    ...
    pdbqtfile=DATA + "/7pn1.pdbqt"
    if not os.path.exists(pdbqtfile):
        pdbfile=hydra.utils.get_original_cwd()+config["smina"]["pep"]
        os.system('obabel {} -O {} -xh'.format(pdbfile, pdbqtfile))
        #pdb_to_pdbqt(hydra.utils.get_original_cwd()+config["smina"]["pep"], DATA + "/7pn1.pdbqt")
    ...
    try:
        ...
        split_sdf_file(DATA + "/dockingpep_poses.sdf")
        return float(l[1])
    except:
        return 0
...
if __name__ == "__main__":
    ...
    #pdb_to_pdbqt(DATA + "/7pn1.pdb", DATA + "/7pn1.pdbqt")

    print(getpepdata("CC(C(=O)NC(CCC(=O)O)C(=O)N)NC(=O)C(C)OC1C(C(OC(C1O)CO)O)NC(=O)C"))
    split_sdf_file(DATA + "/dockingpep_poses.sdf")
