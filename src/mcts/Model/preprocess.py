import sys
import os
append_path = os.path.join(os.path.dirname(__file__), '..')
...
def main(cfg: DictConfig):
    # Loading data
    smiles_list = read_smilesset(hydra.utils.get_original_cwd()+'/data/'+cfg["prep"]["datapath"])
    ...
    # Saving data
    outpath = cfg["prep"]["outdir"]
    with open(f"fragments.smi", mode="w") as f:
        for s in fragment_list:
            f.write(s + "\n")

    with open(f"validation.smi", mode="w") as f:
