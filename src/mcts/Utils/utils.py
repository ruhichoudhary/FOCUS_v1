...
class Node:
    ...
    def calc_UCB(self): 
        if self.visit == 0:
            ...
            #UCB = average reward under node (local exploitation) + visits to node (global exploration). C is how much importance to set to visit. 
            ucb = self.cum_score/self.visit + self.c*math.sqrt(2*math.log(self.parent.visit)/self.visit) #second term will give higher score for less visited node 
        return ucb
